...
def process_csv(category):
    ...
    output_csv_file = f"/media/sean/MusIX/Coqui-AI/Slovak/{category}/{category}_slovak_paths.csv"
    with open(output_csv_file, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Slovak Audio"])  # Write the header row
        ...
        with open(input_csv_file, 'r', encoding='utf-8') as csvfile:
            ...
            for i, row in enumerate(reader):
                ...
                writer.writerow([slovak_filename])

    # # Combine the audio files using FFmpeg
    # combine_audio_files(category, output_csv_file)

    print(f"Slovak audio paths extracted to {output_csv_file}!")
